...
from ParticleModelFuncs import vector_arrows, particle_tracker
...
epsilon = 0.67
plot_types = ['Cells'] # select what to plot, options are Q (net inflow to cells), Qs (water released from storage), Qx (flow across lateral cell boundaries)
...
figsize = (15,15)
t = np.arange(0,5,0.2) # time to run model over in days
...
slope = 1.8 # topographic slope from upper to lower boundary, 1 = lose as much height as horizontal distance
...
#microbial variables
cell0 = 100000 # initial cell concentration in cell/mL
...
Cells = particle_tracker(Out, U, V, W, t, cell0, cellG, cellD, glacier.SHP, glacier.cryoconite_locations)


plotFigures(x, y, z, plot_types, Out, t, plot_layer, Cells, figsize, savepath)